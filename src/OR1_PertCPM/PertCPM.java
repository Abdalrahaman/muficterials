...
public class PertCPM {
     ...
     * @param maEdges Maximum number of edges
     */
   PertCPM(int maxVertices, int maxEdges) {
     ...
     * @exception IllegalArgumentException thrown if one of the vertices does
     * not exist
     */
     ...
     * @exception IllegalArgumentException thrown if one of the vertices does
     * not exist or edge has not been registered yet
     */
     ...
     * ***************************************************************************
     * ***************************************************************************
     * procedure BellmanFord(list vertices, list edges, vertex source) This
     * implementation takes in a graph, represented as lists of vertices and
     * edges, and modifies the vertices so that their distance and predecessor
     * attributes store the shortest paths.
     *
     ...
     * edge uv in edges: // uv is the edge from u to v u := uv.source v :=
     * uv.destination if u.distance + uv.weight < v.distance: v.distance :=
     * u.distance + uv.weight v.predecessor := u
     *
     * Step 3: check for negative-weight cycles for each edge uv in edges: u :=
     * uv.source v := uv.destination if u.distance + uv.weight < v.distance:
    ...
    private void runBellmanFord() {
        ...
        for (int i = 0; i < nVertices; ++i) // relax every edge in 'edges'
        {
    ...
    /**
     * Populate the array with the path in order to print the critical path in
     * the end
     *
     * @param distance Contains the distances from the main source to all other
     * nodes
     *
     */
    ...
    /**
     * Method for relaxing the neighbors for Dijkstra's algorithm. The weights
     * for the neighbors are recalculated and the edges relabeled.
     *
     ...
     *
     * @param vertex the vertex whose distance to change
     ...
     * (INFINITY).
     *
     * @param from minimum distance from this node onwards.
     ...
     *
     * @param vertex the vertex whose predecessor to change
     * @param predes the new predecessor
     */
     ...
     *
     * @param vertex the vertex whose predecessor to get
...
//    }
    ...
    /**
     * Method to determine the end node of the graph. The only node with edges
     * arriving but not leaving will be the terminal node.
     */
    private void getEnd() {
        for (int i = 0; i < vertices.length; i++) {
            int from = 0;
    ...
    /**
     * The assumption is made that the first node in the file will be the
     * starting node. The problem is that given the array index of the start
     * node has the same value as the value used to fill the instantiated array
     * (i.e. 0).
     */
